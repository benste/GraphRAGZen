...
class EntityExtractionPromptFormatting(MappedBaseModel):
    ...
    Args:
        tuple_delimiter (str, optional): Delimiter between tuples in an output record. Defaults to '<|>'.
        record_delimiter (str, optional):  Delimiter between records. Defaults to '##'.
        completion_delimiter (str, optional): Delimiter when no more entities can be extracted. Defaults to '<|COMPLETE|>'.
        entity_types (List[str], optional):  The types that can be assigned to entities (nodes/edges). Defaults to ['organization', 'person', 'geo', 'event'].
...
class EntityExtractionPromptConfig(MappedBaseModel):
    ...
    Args:
        ...
        formatting (EntityExtractionPromptFormatting, optional): Values used to format the base prompts
    """
