...
def summarize_descriptions(
    ...
    **kwargs: DescriptionSummarizationConfig,
    ...
    for node in graph.nodes(data=True):
        ...
        graph.nodes[entity_name]["description"] = item_summarizer(
            entity_name=entity_name, descriptions=descriptions
        )
        i += 1
    ...
    for edge in graph.edges(data=True):
        ...
        graph.edges[entity_name]["description"] = item_summarizer(
            entity_name=entity_name, descriptions=descriptions
        )
        i += 1
...
def _summarize(
    llm: LLM, prompt: DescriptionSummarizationPromptConfig, max_output_tokens: int = -1
) -> str:
    """Use a LLM to summarize a list of descriptions

    Args:
        llm (LLM)
        prompt (DescriptionSummarizationPromptConfig): Already formatted with the entity name and
            descriptions.
        max_output_tokens (int, optional): Maximum number of tokens a summary can have.
