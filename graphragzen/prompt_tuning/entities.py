...
from graphragzen.typing import GenerateEntityRelationshipExamplesConfig, GenerateDomainConfig, GeneratePersonaConfig, GenerateEntityTypesConfig, EntityRelationshipExamplesConfig, CreateEntityExtractionPromptConfig, CreateEntitySummarizationPromptConfig
...
def generate_persona(
    llm: LLM, domain: str, **kwargs: GeneratePersonaConfig
) -> str:
...
def generate_entity_relationship_examples(
    ...
    **kwargs: GenerateEntityRelationshipExamplesConfig,
    ...
    entity_relations = [llm.run_chat(chat) for chat in tqdm(chats, desc = "generating example entity relations")]
    
    # Format the examples and return
    return [config.example_template.format(n=i+1, input_text=doc, entity_types=entity_types_str, output=entity_relation) for i, (doc, entity_relation) in enumerate(zip(sampled_documents, entity_relations))]
...
def create_entity_extraction_prompt(
    ...
    **kwargs: CreateEntityExtractionPromptConfig,
    ...
    Args:
        llm (LLM)
        Entity types (List[str]): The types of entities to extract (e.g. ['person', 'profession', 'location'])
...
def create_entity_summarization_prompt(persona: str, **kwargs: CreateEntitySummarizationPromptConfig) -> str:
